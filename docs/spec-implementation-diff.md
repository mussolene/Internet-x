...
| Control plane | `Name -> Identity -> Locator -> Path` is explicit; the reference profile requires authenticated registration, networked resolution, and authenticated locator update | `refimpl/controlplane.py` and `refimpl/controlplane_service.py` implement a separate HTTP service for signed registration, `Name -> Identity`, `NodeID -> Locator`, lease expiry, and signed locator updates; `Path` remains architectural only | Match with bounded implementation | Docs now state the service is minimally real but locally trusted |
...
- The reference profile is coherent as a runnable MVP-N1 reference package, but its control plane remains deliberately minimal.
